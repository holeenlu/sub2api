export default {
    accounts: {
      ...
      syncLiveAnthropicModels: '同步实时交集',
      syncLiveAnthropicModelsLoading: '正在查询所选账号...',
      syncLiveAnthropicModelsSuccess: '已从实时交集补入 {count} 个模型（所有账号共同支持 {total} 个）',
      syncLiveAnthropicModelsPartial: '已补入 {total} 个实时模型中的 {count} 个；{failed} 个账号未响应',
      syncLiveAnthropicModelsFailures: '{count} 个账号未响应，其支持的模型未计入：',
      syncLiveAnthropicModelsReplace: '替换为实时列表（将移除 {count} 项）',
      syncLiveAnthropicModelsReplaceConfirm:
        '确定用实时交集替换整份白名单？{count} 个并非所有账号都返回的条目将被移除：{models}',
